...
aren't strictly needed. I'll document those here. Feel free to skip anything
in this section that you don't care about.
...
These manifests are official resources that are agnostic to the CSI driver used,
...
Now, to add an NFS share for the cephfs-test-pvc we created earlier, run the
...
You can add a Service for accessing the dashboard outside the cluster as
documented [in the official docs](https://rook.io/docs/rook/v1.14/Storage-Configuration/Monitoring/ceph-dashboard/#viewing-the-dashboard-external-to-the-cluster).
If you don't want to expose the dashboard outside the cluster network, I
recommend using an alias like this:
...
When run, this alias will pull the auto-generated admin secret from the cluster
and copy it to your clipboard, then start a port forward and open the dashboard
